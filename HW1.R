...
costable_survey =ggplot(survey , aes(x=Exer)) +geom_bar()
costable_survey
...
table(survey$Exer,survey$Height)
...
shapiro.test(my_StudentsPerformance$avg_score)  # p<0.05 不是常態
t.test(avg_score~gender,data=my_StudentsPerformance) # T檢定 p<0.05 不是常態 或 分布不一樣
wilcox.test(avg_score~gender,data=my_StudentsPerformance) # 雙樣本中位數檢定 p<0.05 不是常態
...
### anova不準確所以多做一個檢定
#wilcox.test(avg_score~parental_level_of_education,data=my_StudentsPerformance) #  p<0.05
kruskal.test(avg_score~parental_level_of_education,data=my_StudentsPerformance) #  p<0.05
